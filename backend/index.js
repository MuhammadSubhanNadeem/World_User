...
app.post("/", uploader.single("userImg"), (req, res) => {
    ...
    fs.readFile(file_path, (err, data) => {
        ...
        if (!duplication) {
            ...
            fs.unlink(uploadedFilePath, (err) => {
                if (err) {
                    res.status(500).json({"res": "bad", "cause": "Duplication Err!" , "write": false})
                } else{
                    res.status(500).json({"res": "bad", "cause": "Duplication Err!" , "img_del": true, "write": false})
                }
            })
        }
    })
    ...
    function appendData(data, duplication, userEmail) {
        fs.writeFile(file_path, JSON.stringify(data), (err) => {
            if (err) {
                res.end(JSON.stringify({"res": "bad", "cause": "Writing Err!" , "write": false}))
            } else if(duplication){
                res.end(JSON.stringify({"res": "bad", "cause": "Duplication Err!", "write": true}))
            } else{
                let img_unique_key = crypto.randomBytes(8).toString('hex');
                cloudinary.uploader.upload(uploadedFilePath, {
                    ...
                    if (cl_res.public_id === img_unique_key) {
                        ...
                        fs.readFile(img_file_path, (err, img_data) => {
                            if (err) {
                                res.end(JSON.stringify({"res": "bad", "cause": "Img Uploading Error", "write": true, "img_upload": false}))
                            } else {
                                writeFileFun(JSON.parse(img_data));
                            }
                        })
                        function writeFileFun(user_img_data){
                            console.log({user_email: userEmail, user_cloud_img: cl_res.url}, user_img_data);
                            let writeAbleData = [...user_img_data];
                            
                            writeAbleData.push({"user_email": userEmail, "user_cloud_img": cl_res.url});
                            console.log(writeAbleData);
                            fs.writeFile(img_file_path, JSON.stringify(writeAbleData), (err) => {
                                if (err) {
